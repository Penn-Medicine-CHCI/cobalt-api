...
import com.cobaltplatform.api.integration.twilio.TwilioCallStatus;
import com.cobaltplatform.api.integration.twilio.TwilioCallWebhookRequestBody;
...
@Singleton
@ThreadSafe
public class TwilioCallStatusCallbackResource {
	@Nonnull
	...
	@Inject
	public TwilioCallStatusCallbackResource(@Nonnull MessageService messageService,
	...
	@POST("/twilio/{institutionId}/call-status-callback")
	public void callStatusCallback(@Nonnull HttpServletRequest httpServletRequest,
		...
		getLogger().info("Received call status callback from Twilio. Signature is '{}', idempotency token is '{}', request body is '{}'",
		...
		TwilioCallWebhookRequestBody twilioCallWebhookRequestBody = new TwilioCallWebhookRequestBody(requestBody);

		String vendorAssignedId = twilioCallWebhookRequestBody.getParameters().get("CallSid");
...
//		if (messageLog == null) {
//			getMessageService().createTestMessageLog(MessageTypeId.CALL, messageVendorId, vendorAssignedId);
...
//		}
		...
		if (messageLog == null) {
			...
			TwilioCallStatus twilioCallStatus = twilioCallWebhookRequestBody.getTwilioCallStatus().orElse(null);

			boolean deliverySucceeded = twilioCallStatus == TwilioCallStatus.COMPLETED;

			boolean deliveryFailed = twilioCallStatus == TwilioCallStatus.CANCELED ||
					twilioCallStatus == TwilioCallStatus.FAILED ||
					twilioCallStatus == TwilioCallStatus.BUSY ||
					twilioCallStatus == TwilioCallStatus.NO_ANSWER;
			...
			if (deliverySucceeded) {
				...
				String errorCode = twilioCallWebhookRequestBody.getParameters().get("ErrorCode");
				...
				getLogger().info("Nothing to do for Twilio call status '{}', ignoring...", twilioCallStatus == null ? "[null]" : twilioCallStatus.name());
			}
		}
	}
