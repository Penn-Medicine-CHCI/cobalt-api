...
public enum AppointmentStatusCode {
	...
	@Nonnull
	private static final Map<String, AppointmentStatusCode> CODES_BY_FHIR_VALUE;

	@Nonnull
	private final String fhirValue;

	static {
		Map<String, AppointmentStatusCode> codesByFhirValue = new HashMap<>();

		for (AppointmentStatusCode code : AppointmentStatusCode.values())
			codesByFhirValue.put(code.getFhirValue(), code);

		CODES_BY_FHIR_VALUE = Collections.unmodifiableMap(codesByFhirValue);
	}

	private AppointmentStatusCode(@Nonnull String fhirValue) {
	...
	@Nonnull
	public static Optional<AppointmentStatusCode> fromFhirValue(@Nullable String fhirValue) {
