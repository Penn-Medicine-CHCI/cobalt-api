...
package com.cobaltplatform.api.integration.jwt;
...
/**
 * A JWT verifier that is backed by a JWKS URL.
 *
 * @author Transmogrify, LLC.
 */
@ThreadSafe
public class JwksVerifier implements JwtVerifier {
	@Nonnull
	...
	public JwksVerifier(@Nonnull String jwksUrl) {
	...
	@Override
	public void verifyJwt(@Nullable String jwt) throws JwtVerificationException {
		jwt = trimToNull(jwt);

		if (jwt == null)
			throw new JwtVerificationException("No JWT provided to verify");

		String[] components = jwt.split("\\.");

		if (components.length != 3)
			throw new JwtVerificationException(format("Provided JWT is invalid: %s", jwt));
		...
		if (kid == null)
			throw new JwtVerificationException(format("No kid property found in header for JWT: %s", jwt));
		...
		if (publicKey == null)
			throw new JwtVerificationException(format("Unable to find public key in JWKS URL %s matching kid '%s' for JWT: %s", getJwksUrl(), kid, jwt));

		boolean verified = verifyJwtSignatureWithPublicKey(jwt, publicKey);

		if (!verified)
			throw new JwtVerificationException(format("Unable to verify signature using kid '%s' for JWT: %s", kid, jwt));
	}

	@Nonnull
	protected Boolean verifyJwtSignatureWithPublicKey(@Nonnull String jwt,
																										@Nonnull PublicKey publicKey) throws JwtVerificationException {
		requireNonNull(jwt);
		...
		if (jwtSignatureAlgorithm == null)
			throw new JwtVerificationException(format("Unsupported signature algorithm %s for JWT %s", alg, jwt));

		// Don't allow unsafe JWTs
		if (jwtSignatureAlgorithm == JwtSignatureAlgorithm.NONE)
			throw new JwtVerificationException(format("Cannot use unsafe algorithm %s for JWT %s", alg, jwt));
