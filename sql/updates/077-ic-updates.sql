BEGIN;
SELECT _v.register_patch('077-ic-updates', NULL, NULL);

-- Track patient consent
ALTER TABLE patient_order ADD COLUMN patient_consented BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE patient_order ADD COLUMN patient_consented_by_account_id UUID REFERENCES account(account_id);
ALTER TABLE patient_order ADD COLUMN patient_consented_at TIMESTAMPTZ;

DROP VIEW v_patient_order;

-- Recreate view to add new columns
