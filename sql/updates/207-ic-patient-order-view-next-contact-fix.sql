BEGIN;
SELECT _v.register_patch('207-ic-patient-order-view-next-contact-fix', NULL, NULL);

-- Updated next_contact_type_id and next_contact_scheduled_at to fix issue with 'RESOURCE_FOLLOWUP' and 'OTHER' calculations
-- where they would incorrectly take past scheduled screening sessions into account even if the screening was started.
-- Previous definition is in 206-ic-march-2025-enhancements.sql
