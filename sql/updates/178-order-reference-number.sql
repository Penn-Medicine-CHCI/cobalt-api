...
CREATE SEQUENCE po_reference_number_seq_COBALT_IC START 1;

-- Every order will have a unique reference number + institution_id combination (enforced below by constraint and populated by trigger)
-- This lets each institution have its own numbers that "count up from 1" instead of a globally-visible count
ALTER TABLE patient_order ADD COLUMN reference_number BIGINT;

-- Trigger function which sets the value of reference number
CREATE FUNCTION patient_order_reference_number_before_insert() RETURNS TRIGGER AS $$
BEGIN
    NEW.reference_number = nextval('po_reference_number_seq_' || NEW.institution_id);
    RETURN NEW;
END
$$ LANGUAGE 'plpgsql';

-- Configure trigger
CREATE TRIGGER patient_order_reference_number_before_insert_tg
BEFORE INSERT ON patient_order
FOR EACH ROW EXECUTE PROCEDURE patient_order_reference_number_before_insert();

-- Update existing order data
UPDATE patient_order
SET reference_number = subquery.generated_reference_number
FROM (
  SELECT patient_order_id, nextval('po_reference_number_seq_' || institution_id) AS generated_reference_number
  FROM patient_order
  ORDER BY created, order_date, patient_order_id
) AS subquery
WHERE patient_order.patient_order_id=subquery.patient_order_id;

-- Finish configuration of order reference number by enforcing non-null and uniqueness constraints
ALTER TABLE patient_order ALTER COLUMN reference_number SET NOT NULL;
CREATE UNIQUE INDEX patient_order_reference_number_institution_unique_idx ON patient_order USING btree (reference_number, institution_id);

-- With this new column, we need to recreate our patient order views.

drop view v_patient_order;
drop view v_all_patient_order;

-- Now includes reference_number column
